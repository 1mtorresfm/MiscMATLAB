...
function MatlabTDS3014_OpeningFcn(hObject, ~, handles, varargin)
...
function varargout = MatlabTDS3014_OutputFcn(~, ~, handles)
...
handles.tdiv=str2double(query(s,'HOR:MAI:SCA?'));
...
for ch=1:4,
    ...
    if handles.channels.chanSelect(ch*2-1)=='1',  % If channel is active.
        ...
        handles.channels.scale(ch)=str2double(query(s,['CH' chS ':SCA?']));
        fprintf(s,['DAT:SOU CH' chS]);
        
        fprintf(s,'CURV?');
        s1=fread(s, 2*numCollect+numAdd); %fread is specifically for binary data
        ...
        handles.offset.vshift{ch}=...
            str2double(query(s, ['CH' chS ':POS?'])); %Find vertical shift
        %set(handles.MatlabTDS3014fig, 'CurrentAxes', handles.scopeAx); %this brings the axes to the front!!
        handles.channels.bytevoltconvert{ch}=...
            str2double(query(s,'WFMP:YMU?'));
        handles.channels.vshiftvolts{ch}=...
            handles.channels.scale(ch)*...
            str2double(query(s, ['CH' chS ':POS?']));
        ...
        set(handles.vDivText(ch),'String',...
            num2str(handles.channels.scale(ch)));
    end
...
if str2double(handles.timeshift.state)*1==0 %if delay setting is off
    handles.timeshift.percentmove=str2double(query(s,'HOR:TRIG:POS?')); %done in percents, and shifted
    plot(numCollect*0.01*handles.timeshift.percentmove,1, 'rv', 'MarkerSize',8, 'MarkerFaceColor', 'r');
else %delay setting is on
    handles.timeshift.timemove=str2double(query(s,'Hor:Del:Tim?')); %done in terms of time
...
function savedataPB_Callback(~, ~, handles)
...
%Time Shift
if str2double(handles.timeshift.state)*1==0 %whether or not delay is used
...
data = cell(4,1);  % Preallocate.
for ch=1:4,
    if handles.channels.chanSelect(ch*2-1)=='1',  % If channel is active.
        y=(handles.offset.withoutoffset{ch}-...
            handles.channels.vshiftvolts{ch}); %defines output vector and saves
        data{ch}=y;
    else
        data{ch}=[];
    end
...
fileComment=[comm{1} '; Date:' datestr(clock)]; %#ok<NASGU>
...
function printPB_Callback(~, ~, handles)
...
web('http://www.swarthmore.edu/NatSci/echeeve1/Ref/TDS3014/MatlabTDS3014.html','-browser')
...
mySer = instrhwinfo('serial');
numSer = length(mySer.AvailableSerialPorts);
waitbarInc = 1/(1+numSer);
waitbarVal = waitbarInc;
hWait = waitbar(waitbarVal,'Trying to establish contact');

%Open each serial port in succession;
success = 0;
i = 0;
% Convert unsuccessful read to error temporarily (for try/catch in loop)
   w2e=warning('error', 'instrument:query:unsuccessfulRead');
while ~success,
    i=i+1;
    waitbarVal = waitbarVal + waitbarInc;
    hWait = waitbar(waitbarVal, hWait, ...
        ['Trying ' mySer.AvailableSerialPorts{i}]);
    s=serial(mySer.AvailableSerialPorts{i},...
        'baud',38400,...
        'FlowControl','hardware',...
        'InputBufferSize',50000,...
        'Timeout',1,...
        'ReadAsyncMode','continuous',...
        'Terminator','LF'); %#ok<TNMLP>
    fopen(s);
    ...
    try
        ...
        idString = [];  % If timeout, set idString to null
    end

    if ~isempty(strfind(idString,'3014B')),
        success = 1;
    else
        fclose(s);
        delete(s);
...
close(hWait);
    warning(w2e);  %Convert back to warning.
...
function numPts500_RB_Callback(~, ~, handles)
s=handles.serial;
...
function numPts10000_RB_Callback(~, ~, handles)
s=handles.serial;
set(s,'Timeout',1); %defines 1s as time for timeout
